...
const SET_COLORS = {
  "151":"#B8860B","Scarlet & Violet":"#7B1FA2","Paradox Rift":"#1565C0",
...
const TYPE_COLORS = {
  "Fuego":"#FF4C1A","Eléctrico":"#B8860B","Psíquico":"#B24BF3","Dragón":"#4B7BEC",
...
function Stars({ rating, size=14, interactive=false, onRate }) {
  ...
  return (
    <div style={{ display:"flex", gap:2 }}>
      {[1,2,3,4,5].map(i => (
        <span key={i}
          style={{ fontSize:size, cursor:interactive?"pointer":"default", color:i<=(interactive?hover||rating:rating)?"#B8860B":"#333", transition:"color .1s" }}
...
function SellerBadge({ sellerId, size="sm" }) {
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:4 }}>
      ...
      <span style={{ fontSize:size==="sm"?11:13, color:"#B8860B", fontWeight:700, fontFamily:"'DM Sans',sans-serif" }}>{rep.avg}</span>
...
::-webkit-scrollbar{width:5px;}::-webkit-scrollbar-track{background:#080A12;}::-webkit-scrollbar-thumb{background:#B8860B;border-radius:3px;}
.card{background:#10131F;border:1px solid rgba(255,255,255,.07);border-radius:16px;transition:all .25s;}
.card:hover{border-color:rgba(184,134,11,.4);box-shadow:0 0 28px rgba(184,134,11,.08);transform:translateY(-3px);}
.btn{font-family:'DM Sans',sans-serif;cursor:pointer;border:none;border-radius:10px;font-weight:700;transition:all .2s;}
.btn-gold{background:linear-gradient(135deg,#B8860B,#8B6914);color:#fff;padding:11px 24px;font-size:14px;}
.btn-gold:hover{box-shadow:0 4px 22px rgba(184,134,11,.5);transform:scale(1.04);}
...
.btn-outline{background:transparent;color:#B8860B;border:1px solid #B8860B;padding:10px 20px;font-size:14px;}
...
.select:focus{border-color:#B8860B;}
...
.filter-chip.active{color:#080A12;font-weight:700;border-color:transparent;background:#B8860B;}
.filter-chip:hover{transform:scale(1.04);border-color:rgba(184,134,11,.4);color:#B8860B;}
...
.spinner{width:26px;height:26px;border:3px solid rgba(184,134,11,.2);border-top-color:#B8860B;border-radius:50%;animation:spin .8s linear infinite;}
.autocomplete-item{padding:10px 14px;cursor:pointer;display:flex;align-items:center;gap:10px;transition:background .15s;}
.autocomplete-item:hover{background:rgba(184,134,11,.08);}
...
.upload-zone:hover,.upload-zone.drag{border-color:#B8860B;background:rgba(184,134,11,.04);}
`;
    ...
    <div className="modal-bg" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal">
        ...
        <div style={{textAlign:"center",marginBottom:24}}>
          ...
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,color:"#B8860B",letterSpacing:2}}>TIENDA POKE ROJO</div>
        ...
        <div style={{display:"flex",background:"rgba(255,255,255,.04)",borderRadius:10,padding:4,marginBottom:20}}>
          {["login","register"].map(m=>(
            <button key={m} onClick={()=>{setMode(m);setError("");}} className="btn" style={{flex:1,background:mode===m?"rgba(184,134,11,.15)":"transparent",color:mode===m?"#B8860B":"#555",border:"none",padding:"9px",fontSize:13,fontWeight:700,borderRadius:8}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          {mode==="login"&&<div style={{background:"rgba(184,134,11,.06)",border:"1px solid rgba(184,134,11,.15)",borderRadius:8,padding:"10px 14px",fontSize:12,color:"#888",fontFamily:"'DM Sans',sans-serif"}}>
            💡 Demo: <strong style={{color:"#B8860B"}}>ash@pokemon.com</strong> / <strong style={{color:"#B8860B"}}>pikachu</strong>
          </div>}
      ...
      <div className="modal" style={{maxWidth:420}}>
        {done ? (
          <div style={{textAlign:"center",padding:"32px 0"}}>
            ...
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,color:"#B8860B",letterSpacing:1}}>¡GRACIAS POR TU RESEÑA!</div>
          </div>
        ) : <>
          <button onClick={onClose} className="btn" style={{position:"absolute",top:14,right:14,background:"rgba(255,255,255,.06)",color:"#888",width:30,height:30,borderRadius:8,fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
          ...
          <div style={{marginBottom:18}}>
            ...
            {rating>0&&<div style={{fontSize:12,color:"#B8860B",marginTop:6,fontFamily:"'DM Sans',sans-serif"}}>{["","😞 Muy malo","😕 Malo","😐 Regular","😊 Bueno","🤩 Excelente"][rating]}</div>}
          </div>
    ...
    <div className="modal-bg" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal" style={{maxWidth:560}}>
        ...
        <div style={{display:"flex",gap:16,alignItems:"center",marginBottom:24}}>
          <div style={{width:64,height:64,borderRadius:"50%",background:"linear-gradient(135deg,#B8860B,#8B6914)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,fontWeight:700,color:"#fff",flexShrink:0}}>{seller.name[0]}</div>
          <div style={{fontFamily:"'DM Sans',sans-serif"}}>
            <div style={{fontWeight:700,fontSize:20}}>{seller.name}</div>
            <div style={{color:"#555",fontSize:13,marginBottom:4}}>📍 {seller.province}</div>
            {rep.count?(<div style={{display:"flex",alignItems:"center",gap:8}}><Stars rating={Math.round(rep.avg)} size={16}/><span style={{color:"#B8860B",fontWeight:700,fontSize:14}}>{rep.avg}</span><span style={{color:"#555",fontSize:12}}>· {rep.count} reseña{rep.count!==1?"s":""}</span></div>):<span style={{color:"#444",fontSize:13}}>Sin reseñas aún</span>}
          </div>
          <div style={{marginLeft:"auto",textAlign:"center"}}>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:28,color:"#B8860B"}}>{sellerCards.length}</div>
        ...
        <div style={{display:"flex",borderBottom:"1px solid rgba(255,255,255,.07)",marginBottom:20}}>
          {["cartas","reseñas"].map(t=>(
            <button key={t} onClick={()=>setTab(t)} style={{background:"none",border:"none",color:tab===t?"#B8860B":"#555",fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:14,cursor:"pointer",padding:"14px 18px",borderBottom:tab===t?"2px solid #B8860B":"2px solid transparent",transition:"all .2s",textTransform:"capitalize"}}>
            ...
            {sellerCards.map(c=>(
              <div key={c.id} style={{display:"flex",gap:12,alignItems:"center",background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.06)",borderRadius:12,padding:12}}>
                ...
                <div style={{textAlign:"right"}}>
                  <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:"#B8860B"}}>{fmt(c.price)}</div>
      ...
      <div className="modal" style={{maxWidth:440}}>
        {step===1&&<>
          ...
          <div style={{background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.07)",borderRadius:12,padding:14,marginBottom:16,fontFamily:"'DM Sans',sans-serif"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",fontWeight:700}}><span style={{fontSize:15}}>Total</span><span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,color:"#B8860B"}}>{fmt(total)}</span></div>
          </div>
        ...
        {step===2&&<div style={{textAlign:"center",padding:"44px 20px"}}>
          ...
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,color:"#B8860B",marginBottom:8,letterSpacing:2}}>PROCESANDO PAGO</div>
  ...
  if (step===1) return (
    <div style={{maxWidth:500,margin:"60px auto",textAlign:"center"}}>
      <div className="card" style={{padding:48}}>
        ...
        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:30,color:"#B8860B",marginBottom:8,letterSpacing:1}}>¡CARTA PUBLICADA!</div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        <div>
          ...
          <div style={{position:"relative"}}>
            ...
            {suggestions.length>0&&(
              <div style={{position:"absolute",top:"calc(100% + 4px)",left:0,right:0,background:"#14172A",border:"1px solid rgba(184,134,11,.2)",borderRadius:12,zIndex:50,overflow:"hidden",boxShadow:"0 8px 32px rgba(0,0,0,.6)"}}>
                {suggestions.map(s=>(
          ...
          <div style={{display:"flex",gap:16,background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.07)",borderRadius:14,padding:16,alignItems:"flex-start"}}>
            <div style={{flexShrink:0,width:100}}>
              ...
              <div style={{display:"flex",gap:6,marginTop:10}}>
                {form.imgUrl&&<button className="btn" onClick={()=>setImgMode("official")} style={{flex:1,padding:"5px",fontSize:10,background:imgMode==="official"?"rgba(184,134,11,.15)":"rgba(255,255,255,.04)",color:imgMode==="official"?"#B8860B":"#666",border:`1px solid ${imgMode==="official"?"#B8860B":"rgba(255,255,255,.1)"}`,borderRadius:6}}>Oficial</button>}
                <button className="btn" onClick={()=>fileRef.current?.click()} style={{flex:1,padding:"5px",fontSize:10,background:imgMode==="upload"?"rgba(184,134,11,.15)":"rgba(255,255,255,.04)",color:imgMode==="upload"?"#B8860B":"#666",border:`1px solid ${imgMode==="upload"?"#B8860B":"rgba(255,255,255,.1)"}`,borderRadius:6}}>📷 Tuya</button>
              </div>
        ...
        {!selectedCard&&!form.uploadedImg&&(
          <div className={`upload-zone${dragOver?" drag":""}`}
            ...
            <div style={{fontFamily:"'DM Sans',sans-serif",color:"#555",fontSize:13}}>Arrastrá una foto o <span style={{color:"#B8860B",cursor:"pointer"}}>hacé clic para subir</span></div>
        ...
        {Number(form.price)>0&&(
          <div style={{background:"rgba(184,134,11,.05)",border:"1px solid rgba(184,134,11,.12)",borderRadius:9,padding:"12px 14px",fontSize:13,fontFamily:"'DM Sans',sans-serif"}}>
            ...
            <div style={{borderTop:"1px solid rgba(184,134,11,.1)",paddingTop:8,display:"flex",justifyContent:"space-between",fontWeight:700}}>
              <span>Total comprador</span><span style={{color:"#B8860B"}}>{fmt(Math.round(Number(form.price)*(1+COMMISSION)))}</span>
            </div>
...
function CardItem({ card, userId, onBuy, onLogin, onSellerClick }) {
  ...
  return (
    <div className="card" style={{padding:0,overflow:"hidden",display:"flex",flexDirection:"column",position:"relative"}}>
      {card.hot&&<span style={{position:"absolute",top:10,right:10,background:"linear-gradient(135deg,#B8860B,#8B6914)",color:"#fff",padding:"2px 8px",borderRadius:5,fontSize:10,fontWeight:700,fontFamily:"'DM Sans',sans-serif",zIndex:2}}>🔥 HOT</span>}
      ...
      <div style={{padding:"14px 14px 16px",display:"flex",flexDirection:"column",flex:1,fontFamily:"'DM Sans',sans-serif"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:"auto"}}>
          <div>
            ...
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:26,color:"#B8860B",lineHeight:1}}>{fmt(card.price)}</div>
          </div>
          {userId==null
...
export default function App() {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#080A12",color:"#E8E8F0",fontFamily:"'DM Sans',sans-serif",position:"relative"}}>
      <style>{CSS}</style>
      <div style={{position:"fixed",top:-280,right:-180,width:560,height:560,borderRadius:"50%",background:"radial-gradient(circle,rgba(184,134,11,.06) 0%,transparent 70%)",pointerEvents:"none",animation:"glow 5s ease-in-out infinite"}}/>
      ...
      <nav style={{position:"sticky",top:0,zIndex:50,borderBottom:"1px solid rgba(255,255,255,.06)",padding:"0 24px",height:60,display:"flex",alignItems:"center",justifyContent:"space-between",background:"rgba(8,10,18,.92)",backdropFilter:"blur(20px)"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer"}} onClick={()=>setTab("marketplace")}>
          <PokeBall />
          <div>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,letterSpacing:2,color:"#B8860B",lineHeight:1}}>TIENDA POKE ROJO</div>
          ...
          {user?<>
            ...
            <div style={{position:"relative"}}>
              <button onClick={()=>setMenuOpen(o=>!o)} className="btn btn-ghost" style={{padding:"7px 14px",fontSize:13,display:"flex",alignItems:"center",gap:8}}>
                <div style={{width:26,height:26,borderRadius:"50%",background:"linear-gradient(135deg,#B8860B,#8B6914)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:"#fff",flexShrink:0}}>{user.name[0]}</div>
                {user.name.split(" ")[0]}
              </button>
              {menuOpen&&(
          ...
          <div style={{padding:"30px 0 22px",display:"flex",gap:24,alignItems:"center",flexWrap:"wrap",borderBottom:"1px solid rgba(255,255,255,.05)",marginBottom:22}}>
            <div style={{flex:1,minWidth:240}}>
              <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:50,lineHeight:.88,marginBottom:10}}>
                <span style={{color:"#B8860B"}}>COMPRÁ</span> Y <span style={{color:"#B8860B"}}>VENDÉ</span><br/>
            ...
            <div style={{display:"flex",gap:10}}>
              {[{v:`${cards.length}`,l:"Cartas",i:"🃏"},{v:`${USERS.length}`,l:"Vendedores",i:"👤"},{v:`${REVIEWS.length}`,l:"Reseñas",i:"⭐"}].map(s=>(
                <div key={s.l} className="card" style={{padding:"14px 18px",textAlign:"center",minWidth:88}}>
                  ...
                  <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:24,color:"#B8860B"}}>{s.v}</div>
          ...
          <div style={{display:"flex",gap:8,marginBottom:20,overflowX:"auto",paddingBottom:4}}>
            {SETS.map(s=>(
              <button key={s} className={`filter-chip ${filterSet===s?"active":""}`}
                style={filterSet===s?{background:"#B8860B",color:"#080A12",borderColor:"transparent"}:{}}
              ...
              {purchases.map((c,i)=>(
                <div key={i} className="card" style={{padding:14,display:"flex",gap:12,alignItems:"center"}}>
                  ...
                  <div style={{textAlign:"right",marginRight:8}}>
                    <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:"#B8860B"}}>{fmt(Math.round(c.price*(1+COMMISSION)))}</div>
